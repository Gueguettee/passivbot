...
print("=" * 60, flush=True)
print("META-OPTIMIZER MONITOR", flush=True)
print("=" * 60, flush=True)
...
try:
    while True:
        ...
        sys.stdout.flush()
        print("=" * 60, flush=True)
        print("META-OPTIMIZER MONITOR -", datetime.now().strftime("%H:%M:%S"), flush=True)
        print("=" * 60, flush=True)
        ...
        print(f"Status: {status_color}{status}\033[0m", flush=True)

        # Resource usage
        cpu, mem_pct, mem_used, mem_total = get_resource_usage()
        print("Resources:", flush=True)
        print(f"  CPU: {cpu:.1f}%", flush=True)
        print(f"  Memory: {mem_used}MB / {mem_total}MB ({mem_pct:.1f}%)", flush=True)

        # Progress
        results_dir = get_latest_results_dir()
        if results_dir:
            print("Results dir:", results_dir.name, flush=True)
            progress = get_progress(results_dir)
            if progress:
                print("Progress:", flush=True)
                phase = progress.get("phase", "unknown")
                folds = progress.get("folds_completed", 0)
                configs = progress.get("configs_found", 0)
                vals = progress.get("validations_done", 0)
                print("  Phase:", phase, flush=True)
                print("  Folds completed:", folds, flush=True)
                print("  Configs found:", configs, flush=True)
                print("  Validations done:", vals, flush=True)
        else:
            print("No results directory found yet", flush=True)

        print("-" * 60, flush=True)
        print("Press Ctrl+C to exit monitor", flush=True)
        print("To attach to session: tmux attach -t meta_opt", flush=True)
        sys.stdout.flush()
...
'
    ...
    remote_exec "cd $REMOTE_DIR && python3 -u -c '$monitor_script'"
}
...
cmd_download() {
    ...
    local latest_local=$(ls -td "$output_dir"/*/ 2>/dev/null | head -1)
    if [ -n "$latest_local" ] && [ -f "$latest_local/reports/summary.txt" ]; then
        echo ""
        cat "$latest_local/reports/summary.txt"
        echo ""
        log_info "Best config: $latest_local/best_configs/rank_1.json"
    fi
}

cmd_results() {
    log_info "Fetching results from $REMOTE_HOST"
    ...
    echo ""
    echo "=== Latest Run: $dir_name ==="
    echo ""

    # Check if summary.txt exists (generated by ReportGenerator)
    local has_report=$(remote_exec "test -f $latest_dir/reports/summary.txt && echo yes || echo no" | tr -d '[:space:]')

    if [ "$has_report" = "yes" ]; then
        remote_exec "cat $latest_dir/reports/summary.txt"
    else
        # Fallback: parse summary.json with inline Python
        local results_script='
import json, sys
try:
    with open(sys.argv[1]) as f:
        data = json.load(f)
    top = data.get("top_configs", [])
    total = data.get("total_configs_evaluated", 0)
    elapsed = data.get("elapsed_seconds", 0)
    hours = int(elapsed // 3600)
    mins = int((elapsed % 3600) // 60)
    print("=" * 60)
    print("META-OPTIMIZATION RESULTS SUMMARY")
    print("=" * 60)
    print()
    print(f"Total configs evaluated: {total}")
    print(f"Elapsed time: {hours}h {mins}m")
    print()
    print("TOP 5 ROBUST CONFIGURATIONS")
    print("-" * 40)
    for i, c in enumerate(top[:5], 1):
        score = c.get("overall_score", 0)
        adg = c.get("mean_adg", 0)
        sharpe = c.get("mean_sharpe", 0)
        passes = c.get("passes_thresholds", False)
        print(f"  Rank {i}: score={score:.4f}  ADG={adg:.6f}  Sharpe={sharpe:.2f}  pass={passes}")
    print()
    print("=" * 60)
except Exception as e:
    print(f"Error reading results: {e}")
'
        remote_exec "cd $REMOTE_DIR && python3 -u -c '$results_script' $latest_dir/summary.json"
    fi

    echo ""
    log_info "Best config: $latest_dir/best_configs/rank_1.json"
